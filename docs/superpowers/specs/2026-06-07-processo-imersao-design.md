...
| 13 | Fase 4 deploy-readiness **unverified**; onboarding said vague "plano pago" | **Verified live** (scaffold → build → migrate → run): `npm run build` passes, `next start` honors `PORT`, `npx drizzle-kit migrate` (the Railway pre-deploy command) applies migrations, `/api/health` returns `{db:true}`. Railway flow + pricing confirmed against current docs: onboarding now states **Hobby $5/mo (card required) + free Trial (no card)**; `publicar.md` uses the **Add Reference** picker + current menu labels; DB routes carry `dynamic = 'force-dynamic'` so a cold Railway build doesn't break. |
| 14 | Installer's plugin auto-install **failed on fresh Macs** | Root cause found by isolated repro (`CLAUDE_CONFIG_DIR` sandbox): `claude plugin marketplace add owner/repo` clones via **SSH** (`git@github.com:…`), which fails on a student's fresh Mac with no SSH key. Fix: use the **HTTPS URL** everywhere (`add https://github.com/parisgroup-ai/imersao-ia-setup`) — proven to clone via HTTPS in a fresh sandbox. Step 8 (renumbered to step 9 in `1.5.0`) also retries 2× and **prints the real error** instead of swallowing it with `2>/dev/null`. |

Version trail: `1.0.0` → `1.1.0` (pipeline ship) → `1.2.x` → `1.2.1` (bússola + narração + Fase-2 single-session collapse) → **`1.2.2`** (E2E hardening: real Design OS export path `product-plan/`, in-place scaffold, dev-server restart for new files, Fase-3 scaffold flags/`.env`/Postgres-readiness, start↔goal clarity) → `1.3.0` (Fase 1 upgraded to a richer product plan; artifact renamed `docs/plano-do-produto.md`) → `1.4.0` (Fase 4 deploy to Railway + onboarding for GitHub/Railway/Claude-$200) → `1.4.1` (Fase 3 skips the redundant design gate; Fase 4 deploy-readiness hardening) → `1.4.2` (deploy verified live; Railway pricing/flow corrected to current docs; `force-dynamic` for DB routes) → **`1.4.3`** (installer plugin-install fixed: HTTPS clone instead of SSH; retry + unmasked error) → **`1.5.0`** (installer adds the **ToStudy CLI** `@tostudy-ai/cli` as step 8 — the public npm course CLI used during the imersão; plugin/skills step renumbered to 9, final verify to 10; `check.sh` + README + recovery docs updated to match) → **`1.6.0`** (installer adds the **ParisGroup statusline** `@parisgroup-ai/claude-statusline` — installed from the **public Git repo** because the npm package lives on GitHub Packages and would need a token on a fresh Mac; auto-wires `~/.claude/settings.json` via `jq` only when `.statusLine` is absent; also installs MesloLG Nerd Font + `jq`; new **step 10**, final verify renumbered to **11**; statusline checked by `command -v`, never `--version`, because the script reads stdin and would hang) → **`1.6.1`** (fixes 1.6.0: `npm install -g github:parisgroup-ai/claude-statusline` returns exit 0 but installs a **broken bin** — a dangling symlink whose target the git-install never materializes, so `claude-statusline` is command-not-found / exit 127 on a clean PATH. The statusline is a single self-contained bash script, so the installer now **curls it** to `~/.npm-global/bin/claude-statusline` and `chmod +x`; install + both verify gates run a **real smoke test** — pipe statusLine JSON to the bin and require exit 0 — instead of presence-only, which would mask a broken bin). User-facing commands resolve under the plugin namespace as `/imersao:pg-imersao-*` (the bare `/pg-imersao-*` form does not resolve).
...
plugins/imersao/.claude-plugin/plugin.json      version 1.0.0 → 1.6.1
